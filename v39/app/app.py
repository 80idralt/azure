...
Om FLOW_URL är satt postas ärendet dessutom direkt till Power Automates
HTTP-trigger. HTTP-vägen valdes framför blob-triggern eftersom
lagringskontots namn byts vid varje omdeploy (uniqueString), medan
flödets URL är stabil oavsett hur många gånger miljön rivs och byggs om.
...
FLOW_URL = os.environ.get("FLOW_URL", "")           # Power Automate HTTP-trigger
# -------------------------------------------------------------------------
...
def notifiera_flode(arende):
    """Postar ärendet till Power Automate, om FLOW_URL är konfigurerad.

    Inslaget i try/except: ett nere flöde eller en felaktig URL ska aldrig
    hindra ärendet från att sparas, formuläret måste fungera ändå.
    """
    if not FLOW_URL:
        return
    try:
...
def health():
    ...
    return {
        "status": "ok",
        "konto": STORAGE_ACCOUNT,
        "container": CONTAINER,
        "flode_konfigurerat": bool(FLOW_URL),
    }
